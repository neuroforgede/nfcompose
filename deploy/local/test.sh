...
HTTP_CODE=$(docker run --rm --network "${COMPOSE_PROJECT_NAME}_nfcompose" --rm python:3.11.15 bash -c 'exec curl -o /dev/null -w '%{http_code}' -s http://admin:admin@skipper.test.local:8000/api/')
...
docker run --rm --network "${COMPOSE_PROJECT_NAME}_nfcompose" -v "$(pwd)/integration:/integration" --rm python:3.11.15 bash -c 'cp -r /integration /tests && cd /tests && bash create_venv.sh && source venv/bin/activate && pip3 install -r requirements.txt && exec bash test.sh'
