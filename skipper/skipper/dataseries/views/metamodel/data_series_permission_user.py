...
from skipper.core.utils.permissions import perms_for_user, directly_assigned_perms_for_user, get_assignable_permissions
...
class DataSeriesPermissionUserIdentityField(MultipleParameterHyperlinkedIdentityField):
...
class DataSeriesPermissionUserViewSet(
    CustomizableBrowsableAPIRendererObjectMixin,
    RetrieveModelMixin,
    UpdateModelMixin,
    ListModelMixin,
    ...
    """
    API to define the permissions on this dataseries per User.
    ...
    """
    skipper_base_name = constants.data_series_permission_user_base_name
    ...
    def get_name_string(self) -> str:
        ...
        if 'pk' in self.kwargs:
            return f'{_ds_object.name} - User Permissions: {self.get_object().username}'
        else:
            return f'{_ds_object.name} - User Permissions'
    ...
    def get_serializer_class(self) -> Any:
        ...
        class DataSeriesPermissionSerializer(BaseSerializer):
            url = DataSeriesPermissionUserIdentityField(
                view_name=constants.data_series_permission_user_base_name + '-detail',
                lookup_field='pk'
            )
            ...
            def to_representation(self, obj: Any) -> Any:
                ...
                representation = {
                    'url': self.get_sub_url(
                        constants.data_series_permission_user_base_name + '-detail',
                        [data_series_obj.id, obj.id]
                    ),
                    'id': obj.id,
                    'username': _name,
                    'fully_qualified': obj.username,
                    'user_permissions': sorted(list(directly_assigned_perms_for_user(
            ...
            def update(self, instance: Model, validated_data: Dict[str, Any]) -> Model:
                with transaction.atomic():
                    ...
                    _directly_assigned = directly_assigned_perms_for_user(
                        'dataseries', 
                        cast(User, instance), 
                        data_series_obj, 
                        all_perms_without_prefix
                    )

                    to_remove = (_directly_assigned - new_permissions)
                    to_add = (new_permissions - _directly_assigned)

                    permissions_we_dont_have = (to_remove - assignable_permissions).union(
