...
def migrations_for_datapoint_data_tables(
        data_name: str,
        data_foreign_key: str,
        value_column_def: str,
        fact_or_dim_id: str = 'fact_id',
        value_name: str = 'value'
) -> List[migrations.operations.base.Operation]:
    ...
    return [
        ...
        # and also, if we do not have the foryeign keys on the data point,
        ...
        migrations.RunSQL(
            ...
            (
                {fact_or_dim_id} uuid NOT NULL,
                data_point_id {data_point_id_column_def} NOT NULL,
                point_in_time timestamp with time zone NOT NULL,
                {value_name} {value_column_def}
            ) PARTITION BY LIST({fact_or_dim_id});
            
            CREATE UNIQUE INDEX {table_name}_data_point_fkey
                ON {table_name} USING btree
                ({fact_or_dim_id}, data_point_id COLLATE pg_catalog."default", point_in_time);
"""
        ), *create_views_for_datapoint_data_tables_initial(
            data_name=data_name,
...
def drop_views_for_datapoint_data_tables(
        data_name: str
) -> List[migrations.operations.base.Operation]:
    relevant_table_name = f'data_point_{data_name}'
...
def create_views_for_datapoint_data_tables_initial(
        data_name: str,
        fact_or_dim_id: str = 'fact_id',
        value_name: str = 'value'
) -> List[migrations.operations.base.Operation]:
...
def create_views_for_datapoint_data_tables(
        data_name: str,
        fact_or_dim_id: str = 'fact_id',
        value_name: str = 'value'
) -> List[migrations.operations.base.Operation]:
