        ...
        if dataseries.backend == 'DYNAMIC_SQL_NO_HISTORY' or dataseries.backend == 'DYNAMIC_SQL_MATERIALIZED':
            if dataseries.metamodel_version <= 3:
                ...
                with connections[DATA_SERIES_DYNAMIC_SQL_DB].cursor() as cursor:
                    ...
                    if exists:
                        cursor.execute(f"""
                            ALTER TABLE {schema_name}.{table_name} DROP CONSTRAINT IF EXISTS {escape.escape(f'_mat_external_id_uniq_{str(dataseries.id)}_{dataseries.external_id}')};
                            """)
                ...
                dataseries.metamodel_version = 4
                dataseries.save()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ('skipper_dataseries_storage_dynamic_sql', '0023_add_property_external_id_index_on_materialized_table'),
    ]

    operations = [
        RunPython(migrate_materialized_dataseries_drop_unique_external_id),
    ]
